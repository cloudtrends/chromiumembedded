...
void CefProcess::CreateIOThread() {
  ...
  scoped_ptr<base::Thread> thread(new CefProcessSubThread(CefThread::IO));
  base::Thread::Options options;
...
void CefProcess::SetIPCLoggingEnabled(bool enable) {
  // First enable myself.
  if (enable)
  ...
  io_thread()->message_loop()->PostTask
      (FROM_HERE,
       NewRunnableMethod(
           this,
           &CefProcess::SetIPCLoggingEnabledForChildProcesses,
           enable));
}

// Helper for SetIPCLoggingEnabled.
void CefProcess::SetIPCLoggingEnabledForChildProcesses(bool enabled) {
  DCHECK(CefThread::CurrentlyOn(CefThread::IO));
/*
  ChildProcessHost::Iterator i;  // default constr references a singleton
  while (!i.Done()) {
    i->Send(new CefMsg_SetIPCLoggingEnabled(enabled));
    ++i;
  }
*/
}
